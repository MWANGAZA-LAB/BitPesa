...
async function bootstrap() {
  ...
  if (configService.get('ENABLE_SWAGGER', true)) {
    const config = new DocumentBuilder()
      .setTitle('BitPesa M-Pesa Service API')
      .setDescription('M-Pesa integration service for BitPesa Bridge')
      .setVersion('1.0')
      .addTag('mpesa')
      .addTag('stk-push')
      .addTag('b2c')
  ...
  logger.log(`M-Pesa Service is running on: http://localhost:${port}`);
  logger.log(`Swagger documentation: http://localhost:${port}/api/docs`);
  logger.log(`Health check: http://localhost:${port}/health`);
}

bootstrap().catch((error) => {
  console.error('Failed to start M-Pesa Service:', error);
  process.exit(1);
});
